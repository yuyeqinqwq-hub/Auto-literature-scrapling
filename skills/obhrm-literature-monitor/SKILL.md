...
- For team self-service runs, prefer the GitHub Actions manual workflow over asking every teacher/student to install Codex locally. The preferred deployment model is one central personal-account repository with trusted teachers/students added as collaborators who have enough access to run workflow_dispatch jobs; fork-based runs are only a fallback when central collaborator access is not available.
...
Use `.github/workflows/generate-literature-report.yml` when a collaborator needs to generate a report without local Codex or Python setup. For group use, prefer running this workflow in one central personal-account repository so all reports appear on the same GitHub Pages index and each entry can show the triggering GitHub account. Forks remain a fallback for users who cannot be added as collaborators to the central repository. The workflow exposes a GitHub `Run workflow` form with:
...
GitHub only shows `Run workflow` to users with sufficient permission on that repository. For central collaborator use, invite trusted teachers/students through `Settings` -> `Collaborators and teams` and grant enough access to run manual workflows; `Write` is the practical default for this project because the workflow commits published `site/` updates back to the repository. Configure `Settings` -> `Pages` -> `Build and deployment` -> `Source` as `GitHub Actions` once in the central repository. For users who cannot receive central access, direct them to fork the repository and run the workflow in their fork.

The workflow runs `scripts/run_github_report.py`, which scans, renders standalone HTML, publishes the public copy under `site/reports/<run-folder>/`, uploads Markdown/HTML/CSV/log artifacts to the workflow run, commits `site/`, and deploys `site/` to GitHub Pages. If `public_site_url` is left blank in the workflow form, the workflow computes the running repository's default GitHub Pages URL: `https://<owner>.github.io/<repo-name>/`.
