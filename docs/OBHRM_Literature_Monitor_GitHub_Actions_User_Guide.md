...
- 点击小卡片可以放大查看。
- 小卡片右上角的图标只是“放大查看”入口，不代表趋势方向。
- `candidate appearances` 表示该关键词在候选文献中的年度出现次数，这一行会加粗显示。
- `Peak` 表示该关键词候选文献出现次数最高的年份和当年数量。
- `Combined Keyword Trajectories` 会把多个关键词画在同一张总折线图里，方便比较趋势。

总图有两个显示模式：

- `% of keyword peak`：默认模式。每个关键词都按自己的最高年份缩放到 100%，适合比较走势形状。这个模式可以避免高频关键词把低频关键词压在横轴附近。
- `Raw counts`：原始数量模式。适合查看真实数量差距。

把鼠标移动到总图某一年附近时，tooltip 会显示：

- 该年份每个关键词的候选出现次数
- 该年份该关键词候选集中 OpenAlex `cited_by_count` 最高的文献信息
...
如果只输入一个关键词，报告会保持简单结构。

如果输入多个关键词：

- `At a Glance` 会按关键词分别统计命中文章数、有摘要文章数和缺失摘要文章数。
- `Articles With Abstracts` 会拆成多个 section，例如 `Articles With Abstracts - Asia`。
- `Missing Abstract` 也会拆成多个 section，例如 `Missing Abstract - Engagement`。
- 如果一篇文章同时命中多个关键词，它会出现在多个关键词 section 中。这是为了方便不同老师按自己关心的概念阅读。
